"""Tool selection guides, workflow patterns, and configuration information."""
...
async def get_tool_selection_guide() -> Dict[str, Any]:
    ...
    try:
        return {
            ...
            "decision_tree": {
                "simple_site": {"tool": "crawl_url", "when": "Standard HTML sites"},
                "spa_or_js": {"tool": "crawl_url + wait_for_js", "when": "React/Vue/Angular apps"},
                "difficult_site": {"tool": "crawl_url_with_fallback", "when": "Bot protection sites"},
                "multiple_pages": {"tool": "deep_crawl_site", "when": "Documentation/wiki sites"},
                "ai_extraction": {"tool": "intelligent_extract", "when": "Natural language extraction"},
                "regex_patterns": {"tool": "extract_entities", "when": "Emails, phones, addresses"},
                "structured_schema": {"tool": "extract_structured_data", "when": "JSON schema format"},
                "documents": {"tool": "process_file", "when": "PDF, Word, Excel under 100MB"},
                "large_documents": {"tool": "enhanced_process_large_content", "when": "Large docs needing chunking"},
                "youtube": {"tool": "extract_youtube_transcript", "when": "Video transcripts"},
                "search": {"tool": "search_google", "when": "Search results with URLs"},
                "search_crawl": {"tool": "search_and_crawl", "when": "Search + full page content"},
                "batch": {"tool": "batch_crawl/batch_search_google", "when": "Multiple items"}
            },
            "best_practices": {
